...
// Both the main app (for testing) and the keyboard extension contain a copy of this logic.

enum TemplateEngine {

    // MARK: - Entry point

    static func generate(category: TemplateCategory, for property: KeyboardSafeProperty, employeeName: String? = nil) -> String {
        switch category {
        ...
        case .general:         return general(for: property)
    ...
    static func generalInfo(for property: KeyboardSafeProperty) -> String {
        ...
        if property.status != .available {
            sections.append("⚠️ Esta propiedad está \(property.status.label.lowercased()) y no está disponible actualmente.")
        }

        // Primary body: use full sanitized listing text when available
        let listingLower: String
        if let listing = property.publicListingText, !listing.trimmingCharacters(in: .whitespaces).isEmpty {
            sections.append(listing.trimmingCharacters(in: .whitespaces))
            listingLower = listing.lowercased()
        } else if let desc = property.publicDescription, !desc.trimmingCharacters(in: .whitespaces).isEmpty {
        ...
        // Append structured price only when not already present in the listing body
        ...
        if !hasPriceInListing {
            sections.append("Precio: \(buildPriceSection(for: property))")
        }

        // Append requirements only when not already present
        let hasReqsInListing = listingLower.contains("requisito")
        if !hasReqsInListing {
        ...
        // Append maps links when approved location information is available
    ...
    private static func buildPriceSection(for property: KeyboardSafeProperty) -> String {
    ...
    static func price(for property: KeyboardSafeProperty) -> String {
        ...
        if property.operationType == .rent || property.operationType == .rentOrSale,
           let deposit = property.deposit {
    ...
    static func characteristics(for property: KeyboardSafeProperty) -> String {
        ...
        let bathStr = fmtDouble(property.bathrooms)
        if property.bathrooms > 0 {
            lines.append("• \(bathStr) baño\(property.bathrooms == 1 ? "" : "s")")
        }
        ...
        let appliances = property.includedAppliances.filter { !$0.isEmpty }
        let items = property.includedItems.filter { !$0.isEmpty }
        let included = appliances + items
        if !included.isEmpty {
    ...
    static func pets(for property: KeyboardSafeProperty) -> String {
        ...
        case .allowed:
            return "Se acepta mascota."
        case .notAllowed:
            return "No se aceptan mascotas."
        case .subjectToCaseAnalysis:
            return "Sujeto a análisis de caso."
        }
    }

    // MARK: - Purchase information (sale only)
    ...
    static func purchaseInfo(for property: KeyboardSafeProperty) -> String {
        ...
        case .unavailable:
            ...
            switch property.fhaEligibility {
            case .eligible:
        ...
        if let iusiAmt = property.iusiAmount {
            ...
            if let freq = property.iusiFrequency, !freq.isEmpty {
                iusiPart += " (\(freq))"
            }
            if let verifiedAt = property.iusiVerifiedAt {
                iusiPart += ", verificado el \(fmtDate(verifiedAt))"
            }
            iusiPart += "."
            parts.append(iusiPart)
        }

        if let notes = property.iusiNotes, !notes.isEmpty {
            parts.append(notes)
        }

        if let finNotes = property.financingNotes, !finNotes.isEmpty {
            parts.append(finNotes)
        }
    ...
    static func general(for property: KeyboardSafeProperty) -> String {
        var parts: [String] = ["\(property.displayTitle) (\(property.internalCode))"]

        let label = property.publicLocationLabel ?? property.locationSummary
        parts.append("Ubicado en \(label).")

        let priceStr = fmt(property.price, currency: property.currency)
        switch property.operationType {
        case .rent:      parts.append("Renta: \(priceStr)/mes.")
        case .sale:      parts.append("Precio de venta: \(priceStr).")
        case .rentOrSale: parts.append("Precio: \(priceStr).")
        }
